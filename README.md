...
- `deleteFromBasket()` - удаление товара из корзины, метод переработан, изменен подход к удалению.
...
- `setinBasket()` - установка значения добавленности в корзину, метод переработан, изменен изначальный текст при смене состояния товара.
...
- `productCard:select` - метод, переработан. Добавляет в корзину продукт, без проверки, был ли он уже ранее добавлен, тем самым позволяет добавить несколько товаров одного типа.
